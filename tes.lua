...
BikeLeft:AddToggle('AutoFixBike', {
    ...
    Callback = function(val)
        ...
        local function gearText()
            for _, v in ipairs(plr.PlayerGui:GetDescendants()) do
                if v:IsA('TextLabel') then
                    ...
                    if t:sub(1,1) == '-' and #t <= 3 then return '-' end
                end
            end
            return nil
        end
        ...
        -- restOffsets populated by the heartbeat scanner below.
        local restOffsets = {}  -- [seat] = CFrame (seat-local)

        local fixInProgress = {}

        local function runFix(seat, hum)
            if fixInProgress[seat] then return end
            fixInProgress[seat] = true

            local bw            = findBackWheel(seat.Parent, seat)
            local correctOffset = restOffsets[seat]  -- seat-local CFrame from rest scan
            local pp            = findProximityPrompt(seat.Parent)

            task.spawn(function()
                -- WHEEL FIX: lock back wheel to its pre-mount rest position
                -- for 1.5s every heartbeat so the game's tween cannot win
                if bw and correctOffset then
                    local elapsed = 0
                    local lockConn; lockConn = RunService.Heartbeat:Connect(function(dt)
                        elapsed += dt
                        if elapsed > 1.5 or hum.SeatPart ~= seat then
                            lockConn:Disconnect(); return
                        end
                        -- reconstruct correct world CFrame from seat's live CFrame
                        pcall(function() bw.CFrame = seat.CFrame * correctOffset end)
                    end)
                end

                -- GEAR FIX: wait up to 2s for gear UI, then reseat until 'N'
        ...
        local function setupFixWatcher(char)
            ...
            -- fire immediately if already mounted when toggle was enabled
            local cur = hum.SeatPart
            if cur and cur:IsA('VehicleSeat') then
                runFix(cur, hum)
            end
        ...
        if val then
            -- heartbeat scanner: throttled to every 30 frames (~0.5s)
            -- captures back wheel rest offset BEFORE player mounts
            -- lives inside if val then so only ONE connection ever exists
            local scanFrame = 0
            local bikeName  = plr.Name .. 'sCar'
            if _G.AutoFixScanConn then _G.AutoFixScanConn:Disconnect(); _G.AutoFixScanConn = nil end
            _G.AutoFixScanConn = RunService.Heartbeat:Connect(function()
                scanFrame += 1
                if scanFrame % 30 ~= 0 then return end
                ...
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if isVehicleSeat(obj) and not obj.Occupant then
                        ...
                        while p and p ~= workspace do
                            if p.Name == bikeName then
                                local bw = findBackWheel(p, obj)
                                if bw then
                                    restOffsets[obj] = obj.CFrame:ToObjectSpace(bw.CFrame)
                                end
                                break
                            end
                            p = p.Parent
    ...
    do
            ...
            for name, partData in pairs(raw) do
                ...
                for k, v in pairs(partData) do
                    if type(v) == 'table' and v._c3 then
                        pd[k] = Color3.new(v.r or 0, v.g or 0, v.b or 0)
                    elseif type(v) == 'table' and v._enum then
                        local ok, enumVal = pcall(function()
                            local parts = tostring(v.v):split('.')
                            -- must be exactly "Enum.TypeName.ValueName"
                            if #parts ~= 3 or parts[1] ~= 'Enum' then
                                error('bad enum')
                            end
                            return Enum[parts[2]][parts[3]]
                        end)
                        pd[k] = ok and enumVal or nil  -- skip bad values, don't store raw string
                    else
                        pd[k] = v
                    end
                end
        ...
        local function cfgFlushToDisk(tbl)
            ...
            pcall(function()
                ...
                local json = cfgSerialize(tbl)
                -- sanity round-trip: make sure what we'd write can be read back
        ...
        do
            if type(readfile) == 'function' then
                ...
                elseif not ok then
                    ...
                    -- wipe the bad file so next save starts clean
        ...
        cfgSaveBtn.MouseButton1Click:Connect(function()
            ...
            for _, p in ipairs(model:GetDescendants()) do
                if p:IsA('BasePart') then
                    -- use parent.name..name as key to handle duplicate part names
        ...
        cfgLoadBtn.MouseButton1Click:Connect(function()
            ...
            for _, p in ipairs(model:GetDescendants()) do
                if p:IsA('BasePart') then
                    local key = (p.Parent and p.Parent.Name or '') .. '/' .. p.Name
                    local d = data[key] or data[p.Name]  -- fall back to old name-only key
                    if d then
                        pcall(function()
